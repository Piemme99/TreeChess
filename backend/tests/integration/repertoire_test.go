...
func TestRepertoireRepo_CRUD(t *testing.T) {
	...
	// GetByID
	got, err := repos.Repertoire.GetByID(context.Background(), rep.ID, user.ID)
	require.NoError(t, err)
	...
	_, err = repos.Repertoire.GetByID(context.Background(), rep.ID, user.ID)
...
func TestRepertoireService_AddNode_RealDB(t *testing.T) {
	...
	got, err := svc.GetRepertoire(context.Background(), rep.ID, user.ID)
	require.NoError(t, err)
...
func TestRepertoireService_MergeRepertoires_RealDB(t *testing.T) {
	...
	_, err = svc.GetRepertoire(context.Background(), rep1ID, user.ID)
	assert.ErrorIs(t, err, services.ErrNotFound, services.ErrNotFound)
	_, err = svc.GetRepertoire(context.Background(), rep2ID, user.ID)
...
func TestRepertoireService_ExtractSubtree_RealDB(t *testing.T) {
	...
	origFromDB, err := svc.GetRepertoire(context.Background(), rep.ID, user.ID)
	require.NoError(t, err)
	...
	extractedFromDB, err := svc.GetRepertoire(context.Background(), extracted.ID, user.ID)
	require.NoError(t, err)
...
func TestRepertoireService_UpdateNodeComment(t *testing.T) {
	...
	got, _ := svc.GetRepertoire(context.Background(), rep.ID, user.ID)
...
func TestRepertoireRepo_OptimisticLock_StaleSaveConflicts(t *testing.T) {
	...
	rep, err := repos.Repertoire.Create(context.Background(), user.ID, "Lock Rep", models.ColorWhite)
	require.NoError(t, err)
	...
	first, err := repos.Repertoire.GetByID(context.Background(), rep.ID, user.ID)
	require.NoError(t, err)
	second, err := repos.Repertoire.GetByID(context.Background(), rep.ID, user.ID)
	require.NoError(t, err)
	...
	savedFirst, err := repos.Repertoire.Save(context.Background(), rep.ID, user.ID, firstTree, first.Metadata, first.Version)
	require.NoError(t, err)
	...
	_, err = repos.Repertoire.Save(context.Background(), rep.ID, user.ID, secondTree, second.Metadata, second.Version)
	assert.ErrorIs(t, err, repository.ErrRepertoireConflict)

	// The first writer's data must have survived (no silent lost update).
	final, err := repos.Repertoire.GetByID(context.Background(), rep.ID, user.ID)
	require.NoError(t, err)
	...
	savedRetry, err := repos.Repertoire.Save(context.Background(), rep.ID, user.ID, retryTree, final.Metadata, final.Version)
	require.NoError(t, err)
...
func TestRepertoireRepo_Save_NotFoundVsConflict(t *testing.T) {
	...
	rep, err := repos.Repertoire.Create(context.Background(), user.ID, "NF Rep", models.ColorWhite)
	require.NoError(t, err)

	// Wrong (stale) version on an existing row -> conflict.
	_, err = repos.Repertoire.Save(context.Background(), rep.ID, user.ID, rep.TreeData, rep.Metadata, rep.Version+99)
	assert.ErrorIs(t, err, repository.ErrRepertoireConflict)

	// Deleted row -> not found, regardless of version.
	require.NoError(t, repos.Repertoire.Delete(context.Background(), rep.ID, user.ID))
	_, err = repos.Repertoire.Save(context.Background(), rep.ID, user.ID, rep.TreeData, rep.Metadata, rep.Version)
	assert.ErrorIs(t, err, repository.ErrRepertoireNotFound)
}
...
func TestRepertoireService_AddNode_BumpsVersion(t *testing.T) {
	...
	rep, err := svc.CreateRepertoire(context.Background(), user.ID, "Bump Rep", models.ColorWhite)
	require.NoError(t, err)
	assert.Equal(t, 0, rep.Version)

	updated, err := svc.AddNode(context.Background(), user.ID, rep.ID, models.AddNodeRequest{ParentID: rep.TreeData.ID, Move: "e4", MoveNumber: 1})
	require.NoError(t, err)
